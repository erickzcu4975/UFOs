...
// 1. "variable" creation to keep track of all the "filters" as an object. Create an "empty filters variable" to keep track of all the elements that change when a search is entered. 
// This variable will be used in Step 5 to store the property “id” and the value that was entered from user input.
var filters = [];

// 3. Funtion to update the filters. Inside this function, you’ll write code in Steps 4-5 to update the filters based on user input.
function updateFilters() {

    // 4a. Saving the element that was changed as a variable. Create a variable that saves the "changed element" using d3.select(this).
    ...
    // 6. Calling the function to apply all filters and rebuild the table
    filterTable();
  }
  ...
  function filterTable() {
    ...
    // 8. Set "filtered data" to the "tableData".
    let filteredData = tableData;
 
    // 9. Looping thru all of the filters and keep any data that matches the filter values (from module)
  ...
  // 2. Attaching an event to listen for changes to each filter; from module: d3.selectAll("#filter-btn").on("click", handleClick); 
